...
// Function to validate the user's role
function validateRole(role?: string): void {
	// If a role is provided, validate it
	if (role && !["user", "admin"].includes(role.trim().toLowerCase())) {
		throw new CustomError(ErrorCatalog.ERROR.USER.VALIDATION.INVALID_ROLE);
	}
}
